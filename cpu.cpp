...
const int STATES = 3; // 隱藏狀態數：比對 (M)、插入 (I)、刪除 (D)
const int ALPHABETS = 5; // 可見字母數：A、C、G、T、-

const float LOG_ZERO = -1e30f; // 表示 log(0) 的極小值
...
int main() {
    ...
    for (int len : lengths) {
        ...
        std::cout << "長度: " << len << ", 時間: " << duration.count() << " 秒, Log-Likelihood: " << logLikelihood << std::endl;
    }
