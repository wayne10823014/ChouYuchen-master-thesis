...
As GPU acceleration gains traction in bioinformatics \cite{Banerjee2017,Liu2021-bib}, conventional CUDA and OpenCL workflows still require vendor-specific driver installation and remain tethered to particular hardware, limiting online teaching and front-end clinical analysis. Standardized in 2024, \textbf{WebGPU} unifies Vulkan, Direct3D~12, and Metal under a single JavaScript API \cite{W3C2024-webgpu}, offering three key advantages: no installation, cross-hardware portability, and on-device data residency. Using the compute-intensive Pair-Hidden Markov Model Forward (Pair-HMM Forward) algorithm \cite{Durbin1998} as a case study, we assess the performance and feasibility of this emerging framework.

Building on the open-source C++/CUDA implementation by Chou Yu-Chen \cite{Chou2024-github}, we first develop a WebGPU baseline. We then mitigate its principal bottlenecks—frequent round-trips between the CPU and GPU, and costly BindGroup reconstruction—by introducing (i) batched submission of a single \emph{CommandBuffer} and (ii) Dynamic Uniform Offsets, yielding an optimized variant termed \emph{WebGPU-Optimized}.
...
隨著 GPU 加速在生物資訊領域日益普及\cite{Banerjee2017,Liu2021-bib}，傳統 CUDA／OpenCL 工作流程仍需安裝廠商驅動程式，且受限於特定硬體，難以支援線上教學與前端臨床分析。2024 年標準化的 \textbf{WebGPU} 以單一 JavaScript API 統合 Vulkan、Direct3D~12 與 Metal\cite{W3C2024-webgpu}，具備「免安裝」、跨硬體可攜性與本機資料駐留三大優勢。本研究以運算密集的 Pair-Hidden Markov Model Forward（Pair-HMM Forward）演算法\cite{Durbin1998} 為案例，評估該新框架的效能與可行性。

我們以周育晨公開的 C++／CUDA 原始碼為基準\cite{Chou2024-github}，首先實作 WebGPU Baseline；隨後針對 CPU 與 GPU 之間頻繁往返，以及 BindGroup 重建耗時等瓶頸，依序導入「單一 \emph{CommandBuffer} 批次提交」與「Dynamic Uniform Offsets」，形成 \emph{WebGPU-Optimized}。
...
While mathematically robust, each dynamic-programming (DP) cell requires one \texttt{exp} and one \texttt{log}, hitting WebGPU’s software-emulated SFU pipeline twice per update \cite{NVIDIA2023-cudaguide} (Fig.~\ref{fig:log_exp_pipeline}).
