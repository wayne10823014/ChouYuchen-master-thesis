...
When running high-performance algorithms directly in the browser (for example, WebGPU-accelerated Pair-HMM), small datasets or moderate sequence lengths can be handled entirely on the local GPU, preserving real-time interactivity and low latency. However, for clinical-scale datasets (for instance, millions or tens of millions of bases), the computing resources of a single browser or a single GPU are typically insufficient or would require prohibitively long execution times. To balance “local interactivity” with “large-scale computation,” we propose the concept of distributed execution across edge and cloud: the browser detects the user’s device (edge) GPU capabilities and, based on hardware tier, sequence length, and estimated computational cost, dynamically decides which compute segments should run locally in the browser and which should be delegated to a cloud server or a WebGPU‐serverless platform.

\section*{Conclusion}
The three directions outlined above—enabling true double-precision support, adopting a hybrid WASM+SIMD + WebGPU execution model, and orchestrating distributed edge–cloud pipelines—each address a distinct limitation of the current browser-native Pair-HMM implementation. By closing the FP64 gap, we can guarantee numerical robustness even for ultra-long reads; by leveraging WASM+SIMD as a front-end filter, we can regain low-latency performance on small or fragmented inputs; and by dynamically offloading large sub-workloads to a cloud backend, we can scale to clinical-grade datasets without sacrificing interactivity. Taken together, these future efforts will extend the reach of WebGPU-accelerated genomic workflows beyond prototype demos and interactive teaching, enabling them to operate as production-ready, high-precision, and highly scalable bioinformatics tools.
