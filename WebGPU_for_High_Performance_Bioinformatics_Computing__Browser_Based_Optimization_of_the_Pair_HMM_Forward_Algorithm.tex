...
\begin{itemize}
  ...
  \item Compared with CUDA, WebGPU-Optimized starts at only
        ...
        $0.840$ of CUDA speed (84 %) at $N=10^{5}$.
\end{itemize}
...
\paragraph{SFU Throughput Bottleneck.}
On RTX~2070~S, both CUDA and WebGPU map \texttt{log}/\texttt{exp} to
\texttt{MUFU.LOG/EX2}; the \emph{dependent latency} is therefore the same.
The observed gap stems instead from \emph{throughput}: each
SM sub-partition has a single variable-latency pipeline shared by all SFU
instructions, allowing at most \textbf{one transcendental issue every
four cycles}\.
When three data-dependent \texttt{log}/\texttt{exp} appear back-to-back,
a warp must stall for the pipeline, whereas CUDA kernels
can interleave independent warp work.
